...
export class CeleryPage implements OnInit {
  ...
  isolation: string= " Separate varieties by at least 800 feet-½ mile in their second season of growth.";
  ...
  async isolationModal() {
    const modal = await this.modalController.create({
      component: Details2Page 
    });

    this.sharingService.setDetails(this.isolation);
